...
<div class="home-tour">
    <div class="container"> 	
        <?php                     
            $blog_id = $_GET['id'];
            $query = $this->db->query("SELECT * FROM blog WHERE id=".$blog_id);
            $tours = $query->result_array();
            foreach ($tours as $row1):?>
            <hr>
            <h2 style="font-family: 'Alegreya SC';font-weight: bold;"><?php echo $row1['title'];?></h2>
            <div class="row">
                 <p style="text-align: right;"><a onclick="window.history.back();" style="cursor:pointer;">Click here to Return ...</a></p>
            </div>
            <h4 style="color:#999;"><?php echo $row1['dt'];?></h4>
            <hr>
